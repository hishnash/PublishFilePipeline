...
internal class PipelineState {
    internal static var shared = PipelineState()
    ...
    internal func getRawFiles(for query: FileQuery, with context: PipelineContext) throws -> [PipelineFile] {
        switch query {
        ...
        case .type(let fileExtension, let root):
            ...
            var files: [PipelineFile] = rootFolder.files.recursive.filter { file in
                ...
                let path = PipelinePath(file.path(relativeTo: rootFolder))
    ...
    internal func add(file: File, at path: PipelinePath, with root: PipelinePath) {
        self.queue.sync {
    ...
    public func getStaticOutputs() -> [FileKey: PipelinePath] {
        self.queue.sync {
