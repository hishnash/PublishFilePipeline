...
public struct PipelineTemporaryStageFile: PipelineFile {
    ...
    public init(
        ...
        self.file = PipelineFileWrapper(file: file, path: PipelinePath(file.path(relativeTo: rootFolder)))
    ...
    public init(
        ...
        self.file = PipelineFileWrapper(file: file, path: PipelinePath(file.path(relativeTo: rootFolder)))
        self.canonical = sourceFiles.first?.canonical.replacing(name: named) ?? PipelinePath(named)
    }
